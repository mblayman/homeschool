...
class UnconfiguredHashidSerialField(fields.Field):
    def bind(self, field_name, parent):
        ...
        raise exceptions.ImproperlyConfigured(
            f"The field '{field_name}' on {parent.__class__.__name__} must be explicitly declared when used with a ModelSerializer"
        )


class HashidSerializerMixin:
    ...
    def __init__(self, **kwargs):
        ...
        if source_field:
            ...
            if isinstance(source_field, str):
                ...
                source_field,
                (HashidField, BigHashidField, HashidAutoField, BigHashidAutoField),
            ):
...
class HashidSerializerCharField(HashidSerializerMixin, fields.CharField):
    ...
    def to_internal_value(self, data):
        ...
        if isinstance(data, str) and not self.allow_int_lookup:
            ...
            if (
                _is_int_representation(without_prefix)
                and without_prefix != hashid.hashid
            ):
                self.fail("invalid_hashid", value=data)
        return hashid
